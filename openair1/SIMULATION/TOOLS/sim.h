...
/* BICTR (Lunar Wireless Channel Model) Configuration */
typedef struct {
  // Reflector generation
  int refCount;              // max number of reflectors to generate
  int refAttemptPerRing;     // attempts per ring
  double ringRadiusMin;      // minimum ring radius (meters)
  double ringRadiusMax;      // maximum ring radius (meters)
  double ringRadiusUncertainty; // +- random offset to radius (meters)
  int ringCount;             // Number of rings to try between min and max
  
  // Ground reflection
  double complexRelPermittivityReal;      // real part of permittivity
  double complexRelPermittivityRealStd;   // std dev of real part
  double complexRelPermittivityImag;     // imaginary part of permittivity
  double complexRelPermittivityImagStd;  // std dev of imaginary part
  int horizontalPolarization;            // 1 for horizontal, 0 for vertical
  
  // Rayleigh fading
  int fadingPaths;           // number of fading paths
  double fadingDopplerSpread; // Doppler spread (m/s)
  
  // Terrain/LOS (optional - can be NULL if terrain data not available)
  int useTerrainLOS;         // 1 to use terrain-based LOS, 0 to assume LOS
  void *terrainData;         // pointer to terrain data structure (can be NULL)
} bictr_config_t;
...
/* BICTR-specific parameter names */
#define CHANNELMOD_MODEL_BICTR_REFCOUNT_PNAME "bictr_refCount"
#define CHANNELMOD_MODEL_BICTR_REFATTEMPT_PNAME "bictr_refAttemptPerRing"
#define CHANNELMOD_MODEL_BICTR_RINGRADIUSMIN_PNAME "bictr_ringRadiusMin"
#define CHANNELMOD_MODEL_BICTR_RINGRADIUSMAX_PNAME "bictr_ringRadiusMax"
#define CHANNELMOD_MODEL_BICTR_RINGRADIUSUNCERT_PNAME "bictr_ringRadiusUncertainty"
#define CHANNELMOD_MODEL_BICTR_RINGCOUNT_PNAME "bictr_ringCount"
#define CHANNELMOD_MODEL_BICTR_PERMREAL_PNAME "bictr_complexRelPermittivityReal"
#define CHANNELMOD_MODEL_BICTR_PERMREALSTD_PNAME "bictr_complexRelPermittivityRealStd"
#define CHANNELMOD_MODEL_BICTR_PERMIMAG_PNAME "bictr_complexRelPermittivityImag"
#define CHANNELMOD_MODEL_BICTR_PERMIMAGSTD_PNAME "bictr_complexRelPermittivityImagStd"
#define CHANNELMOD_MODEL_BICTR_HORIZPOL_PNAME "bictr_horizontalPolarization"
#define CHANNELMOD_MODEL_BICTR_FADINGPATHS_PNAME "bictr_fadingPaths"
#define CHANNELMOD_MODEL_BICTR_FADINGDOPPLER_PNAME "bictr_fadingDopplerSpread"
...
#define CHANNELMOD_MODEL_PARAMS_DESC {  \
    ...
    {CHANNELMOD_MODEL_BICTR_REFCOUNT_PNAME, "BICTR: max number of reflectors\n", 0, .iptr=NULL, .defintval=5, TYPE_INT, 0 }, \
    {CHANNELMOD_MODEL_BICTR_REFATTEMPT_PNAME, "BICTR: attempts per ring\n", 0, .iptr=NULL, .defintval=3, TYPE_INT, 0 }, \
    {CHANNELMOD_MODEL_BICTR_RINGRADIUSMIN_PNAME, "BICTR: min ring radius (m)\n", 0, .dblptr=NULL, .defdblval=5.0, TYPE_DOUBLE, 0 }, \
    {CHANNELMOD_MODEL_BICTR_RINGRADIUSMAX_PNAME, "BICTR: max ring radius (m)\n", 0, .dblptr=NULL, .defdblval=300.0, TYPE_DOUBLE, 0 }, \
    {CHANNELMOD_MODEL_BICTR_RINGRADIUSUNCERT_PNAME, "BICTR: ring radius uncertainty (m)\n", 0, .dblptr=NULL, .defdblval=15.0, TYPE_DOUBLE, 0 }, \
    {CHANNELMOD_MODEL_BICTR_RINGCOUNT_PNAME, "BICTR: number of rings\n", 0, .iptr=NULL, .defintval=10, TYPE_INT, 0 }, \
    {CHANNELMOD_MODEL_BICTR_PERMREAL_PNAME, "BICTR: permittivity real part\n", 0, .dblptr=NULL, .defdblval=3.0, TYPE_DOUBLE, 0 }, \
    {CHANNELMOD_MODEL_BICTR_PERMREALSTD_PNAME, "BICTR: permittivity real std dev\n", 0, .dblptr=NULL, .defdblval=0.5, TYPE_DOUBLE, 0 }, \
    {CHANNELMOD_MODEL_BICTR_PERMIMAG_PNAME, "BICTR: permittivity imag part\n", 0, .dblptr=NULL, .defdblval=0.1, TYPE_DOUBLE, 0 }, \
    {CHANNELMOD_MODEL_BICTR_PERMIMAGSTD_PNAME, "BICTR: permittivity imag std dev\n", 0, .dblptr=NULL, .defdblval=0.05, TYPE_DOUBLE, 0 }, \
    {CHANNELMOD_MODEL_BICTR_HORIZPOL_PNAME, "BICTR: horizontal polarization (1=yes, 0=no)\n", 0, .iptr=NULL, .defintval=1, TYPE_INT, 0 }, \
    {CHANNELMOD_MODEL_BICTR_FADINGPATHS_PNAME, "BICTR: number of fading paths\n", 0, .iptr=NULL, .defintval=20, TYPE_INT, 0 }, \
    {CHANNELMOD_MODEL_BICTR_FADINGDOPPLER_PNAME, "BICTR: Doppler spread (m/s)\n", 0, .dblptr=NULL, .defdblval=0.0, TYPE_DOUBLE, 0 }, \
}
// clang-format on
